...
export const Route = createFileRoute("/app/invoices/new")({
  head: () => ({ meta: [{ title: "New invoice — Quoinv" }] }),
...
function NewInvoicePage() {
  ...
  const save = useMutation({
    mutationFn: async (status: "draft" | "sent") => {
      ...
      const invoiceNumber = `${company.invoice_prefix}${String(company.next_invoice_no).padStart(5, "0")}`;
      await supabase.from("companies")
        .update({ next_invoice_no: company.next_invoice_no + 1 })
        .eq("id", profile.company_id);
      ...
      if (itemRows.length > 0) {
        const { error: eErr } = await supabase.from("invoice_items").insert(itemRows);
        if (eErr) throw eErr;
      }
    ...
    onSuccess: (id) => {
      ...
      navigate({ to: "/app/invoices/$id", params: { id } });
    },
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Create and send to your client.</p>
      </div>
